...
public class RedisCircuitBreaker {
    ...
    private static final long OPEN_DURATION_MS = 10_000; // 10초
    ...
    void registerMetrics() {
        meterRegistry.gauge("redis.circuit.state", this, cb -> {
        ...
        return switch (state) {
            ...
            case OPEN -> {
                if (System.currentTimeMillis() - openedAt.get() >= OPEN_DURATION_MS) {
                    ...
                    log.info("[CIRCUIT] OPEN → HALF_OPEN: Redis 1건 시도 허용");
                    yield true;
                }
                yield false;
            }
    ...
    public void recordSuccess() {
        if (state == State.HALF_OPEN) {
            log.info("[CIRCUIT] HALF_OPEN → CLOSED: Redis 복구 확인");
        }
        state = State.CLOSED;
    ...
    private void transitionToOpen() {
        ...
        log.warn("[CIRCUIT] → OPEN: Redis 연속 실패 {}회, {}ms 후 재시도",
